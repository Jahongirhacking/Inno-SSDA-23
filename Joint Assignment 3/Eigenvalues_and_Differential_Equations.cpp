...
Matrix* SeidelMethod(Matrix* A, Matrix* b) {
    Matrix *prev, *current, *D_1, *Identity, *B, *C, *alpha, *beta;
    double epsilon;
    int index = 0;
    //Identity Matrix
    Identity = new IdentityMatrix(A->getHeight());
    ...
    B = getLowerTriangular(alpha);
    cout<<"B:"<<endl;
    B->displayMatrix();
    // find Upper Triangular
    C = getUpperTriangular(alpha);
    cout<<"C:"<<endl;
    C->displayMatrix();
    // find I-B
    cout<<"I-B:"<<endl;
    ((*Identity) - (*B))->displayMatrix();
    // find I-B inverse
    cout<<"(I-B)_-1:"<<endl;
    inverseMatrix((*Identity) - (*B))->displayMatrix();

    B = getLowerTriangular(A);
    C = getUpperTriangular(A);
    prev = beta;
    do {
      current = (*inverseMatrix(B)) * (*((*b) - *((*C) * (*prev))));
      epsilon = findEpsilon((*current) - (*prev));
      cout<<"x("<<index++<<"):"<<endl;
      prev->displayMatrix();
      cout<<"e: "<<epsilon<<endl;
...
int main() {
    ...
    out<<"t: "<<endl;
    rep(i, N+1){
        out<<fixed<<setprecision(PREC)<<moments[i]<<" ";
    }
    out<<endl;
    ...
    out<<"v: "<<endl;
    rep(i, N+1){
        out<<fixed<<setprecision(PREC)<<victims[i]<<" ";
    }
    out<<endl;
    ...
    out<<"k: "<<endl;
    rep(i, N+1){
        out<<fixed<<setprecision(PREC)<<killers[i]<<" ";
    }
